...
class EVMChainCompiler(BaseChainCompiler):
    ...
    def _build_graph(
        ...
        for row in rows:
            ...
            # Apply fiat value filter (best-effort; skip if no fiat data).
            if options.min_value_fiat is not None and value_native is not None:
                # Without a price at the transaction timestamp we cannot filter
                # accurately. Skip only if there is no fiat data at all.
                pass  # TODO Phase 5: integrate price oracle lookup here.
