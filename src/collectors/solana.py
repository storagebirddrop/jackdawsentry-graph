...
class SolanaCollector(BaseCollector):
    ...
    async def get_block(self, slot_number: int) -> Optional[Block]:
        ...
        try:
            ...
            block_data = await self.client.get_block(
                slot_number, encoding="json", max_supported_transaction_version=0
            )
            ...
            return Block(
                ...
                number=slot_number,
                timestamp=(
                    datetime.fromtimestamp(block["block_time"] / 1000)
    ...
    async def get_transaction(self, tx_signature: str) -> Optional[Transaction]:
        ...
        try:
            ...
            if account_keys:
                from_address = account_keys[
                    0
                ]  # First account is typically the fee payer
    ...
    async def get_address_transactions(
        ...
        try:
            ...
            logger.error(
                f"Error getting Solana address transactions for {address}: {e}"
            )
            return []
    ...
    async def parse_token_balances(
        self,
    ...
    async def get_network_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if slot_samples:
                avg_tps = sum(
                    sample.get("numTransactions", 0) for sample in slot_samples
                ) / len(slot_samples)
