...
## 2026-04-09 — Audit a stale alternative branch against main before filing a PR

**What went wrong:** `feat/asset-scoped-expansion-clean` was created as a
cleaner version of the asset-scoped expansion feature. By the time it was
reviewed, the main feature plus all follow-up work (6 regression test files,
ingest hardening, collector improvements) had already merged via separate PRs.
The branch was ~3,860 lines behind main on code; only two doc improvements
were worth carrying forward.

**Rule:** Before filing a PR from a long-lived alternative branch, run
`git diff main...<branch> --stat` and `git log main...<branch> --oneline`.
If the branch is behind main on code and the shared commits already landed,
audit first — don't assume the branch adds value. Check timestamps and stats
against known merged commits before opening a PR that might revert
shipped work.

## 2026-03-28 — The graph compose file is not the whole runtime story by itself

**What went wrong:** It is easy to inspect or test `docker-compose.graph.yml`
in isolation and accidentally reason about graph ingest, collector behavior, or
backfill availability as if the base compose file described the whole runtime.
In this repo, ingest-capable posture is defined by pairing the base compose file
with `docker-compose.graph.ingest.yml`.

**Rule:** Whenever work touches `docker-compose.graph.yml`, explicitly decide
whether the paired ingest overlay `docker-compose.graph.ingest.yml` also
matters. For review, testing, and operational reasoning about ingest behavior,
collector availability, or full graph-runtime posture, treat the two compose
files as a coupled unit rather than evaluating the base file alone.
