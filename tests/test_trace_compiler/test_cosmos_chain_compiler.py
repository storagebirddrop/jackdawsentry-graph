"""
Unit tests for CosmosChainCompiler (src/trace_compiler/chains/cosmos.py).

All DB calls are mocked — no running PostgreSQL or Neo4j required.

Covers:
- supported_chains returns ["cosmos"]
...
- Address normalization (lowercases — bech32 is lowercase by spec)
...
SEED = "cosmos1aaabbbcccdddeeefffggghhh000111222333"
COUNTERPARTY = "cosmos1zzzyyyxxxwwwvvvuuutttsssnnn999888777"
TX_HASH_1 = "tx" + "a" * 62
TX_HASH_2 = "tx" + "b" * 62
...
def _token_row(counterparty, tx_hash=TX_HASH_1, value=100.0, symbol="ATOM"):
    return {
        "counterparty": counterparty,
        "tx_hash": tx_hash,
        "value_native": value,
        "asset_symbol": symbol,
        "canonical_asset_id": "cosmos",
        "timestamp": None,
    }
...
def test_supported_chains():
    compiler = CosmosChainCompiler()
    assert compiler.supported_chains == ["cosmos"]
...
async def test_expand_next_no_pg_returns_empty():
    ...
    compiler = CosmosChainCompiler(postgres_pool=None, neo4j_driver=None)
...
async def test_expand_next_returns_node_per_unique_counterparty():
    ...
    compiler = CosmosChainCompiler(postgres_pool=pg)
...
async def test_token_transfer_rows_produce_nodes():
    """IBC token transfer rows are included in graph output."""
...
async def test_bridge_detection_produces_bridge_node():
    ...
    BRIDGE_ADDR = "cosmos1bridge000000000000000000000000001"
...
def test_normalize_address_lowercases():
    """Cosmos bech32 addresses are lowercased during normalization."""
    compiler = CosmosChainCompiler()
    addr = "COSMOS1AAABBBCCCDDDEEEFFFGGGHHH000111222333"
...
def test_native_symbol_is_atom():
    compiler = CosmosChainCompiler()
    assert compiler._native_symbol("cosmos") == "ATOM"


def test_native_canonical_asset_id_is_cosmos():
    compiler = CosmosChainCompiler()
    assert compiler._native_canonical_asset_id("cosmos") == "cosmos"
